...
namespace DataObjects
{
  ...
  {
    private FileType FileInfo;   // Holds the properties of the file type
    ...
    /// <summary>
    /// Checks whether the registry is already setup according to the passed parameters.
    /// </summary>
    /// <param name="correctCommandLine"></param>
    /// <returns></returns>
    public bool Check (string correctCommandLine)
    {
      ...
      string properName = RegKey.GetValue("").ToString();  // Get Default value
      RegKey.Close();
      if (properName == null || properName != ProperName)
        return false;

      // At this time we're not going to check the Content Type

      RegKey = Registry.ClassesRoot.OpenSubKey(ProperName);
      if (RegKey == null)
        return false;

      string fullName = RegKey.GetValue("").ToString();  // Get Default value
      RegKey.Close();
      if (fullName == null || fullName != FullName)
        return false;

      // At this time we're not going to check anything to do with the icon

      RegKey = Registry.ClassesRoot.OpenSubKey(ProperName + "\\Shell\\open\\Command");
      if (RegKey == null)
        return false;

      string commandLine = RegKey.GetValue("").ToString();  // Get Default value
      RegKey.Close();
      if (commandLine == null || commandLine == "")
        return false;

      if (commandLine != correctCommandLine)
        return false;
